...
public class EmailService {
    ...
    public List<EmailResponseDTO> getEmailDataList(String clientId) {
        return emailRepository.findAllByClientId(clientId).stream() // findAll()은 Iterable 반환, 이를 Stream으로 변환
                .map(EmailResponseDTO::new)  // 조회한 엔티티를 DTO로 변환
                .collect(Collectors.toList());  // 최종적으로 List로 변환
    }

    public EmailResponseDTO getEmailData(Long id,String clientId){
        return emailRepository.findByIdxAndClientId(id,clientId)
                .map(EmailResponseDTO::new)  // 조회한 엔티티를 DTO로 변환
